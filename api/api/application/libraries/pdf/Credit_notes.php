...
class Credit_notes
{
    ...
    public function generate_pdf($ids, $output = 'F', $base_url = '', $echo_json = true)
    {
        ...
        if (is_null($ids)) :
            $return['message'] = 'no credit_note id(s) given';
        ...
        $this->CI->tcpdf->SetTitle('credit_note');
        ...
        foreach ($ids as $entity) {
            ...
            $credit_note_data_ = $this->credit_note_data($id);

            if (!empty($credit_note_data_)) {
                # get credit_note and company data:
                $other_db_data = $credit_note_data_[$id];
                $company_data = $this->company_data();

                $credit_note_number .= '_' . $other_db_data->credit_note_number;
...
EOD;
                ...
                # credit_note title / number
                $credit_note_title = <<<EOD
            <div style="color:$text_color2;">
            <h3 style="font-weight:normal; line-height: normal;">
                <span style="color:$text_color;">CREDIT NOTE</span> #$other_db_data->credit_note_number
            </h3>
            </div>
EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html' => $credit_note_title, 'align' => 'R'));
...
EOD;
                ...
                # credit_note info
                $credit_note_info = <<<EOD
            <div style="font-size:8; font-weight:normal; color:$text_color;">
            <strong>Issued date:</strong> $other_db_data->date <br>
...
EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html' => $credit_note_info, 'align' => 'R', 'y' => 50));
...
EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html' => $actual_credit_note, 'x' => 12, 'y' => 80, 'ln' => 1));
            }
        }
        /*END OF credit_notes LOOP*/

        # set PDF font
        $this->CI->tcpdf->SetFont(PDF_FONT_NAME_MAIN, '', 10);

        //$file_path = FCPATH . 'downloads/pdf/' . md5('credit_note' . $credit_note_number) . '.pdf';
        //$file_path = FCPATH . 'assets/pdf/' . md5('credit_note' . $credit_note_number) . '.pdf';
        if(is_array($original_ids))
        {
            $file_path = FCPATH . 'assets/pdf/' .'bulk_credit_notes_' . create_slug(current_datetime()) . '.pdf';
        }
        else
        {
            $file_path = FCPATH . 'assets/pdf/' . $credit_note_number . '_' . create_slug(current_datetime()) . '.pdf';
        }
        ...
        if (file_exists($file_path)) {
            ...
            $return['message'] = 'credit_note pdf(s) ready for download';
        } else {
            $return['message'] = 'could not export credit_note pdf(s).';
        }
    ...
    public function credit_note_data($credit_note_id = null)
    {
        $params = array(
            'table' => $this->table_prefix . 'credit_notes',
            'entity' => 'credit_note',
            'items_table' => $this->table_prefix . 'credit_note_items'
        );

        $this->CI->load->model('template_model');

        $result = $this->CI->template_model->read($params, $credit_note_id);
