...
class Statements
{
    ...
    public function generate_pdf($ids, $output = 'F', $base_url = '', $echo_json = true)
    {
        ...
        if(is_null($ids)) :
            $return['message'] = 'no invoice id(s) given';
        ...
        $this->CI->tcpdf->SetTitle('Invoice');
        ...
        foreach($ids as $entity)
        {
            ...
            $invoice_data_  = $this->invoice_data($id);

            if(!empty($invoice_data_))
            {
                # get invoice and company data:
                $invoice_db_data = $invoice_data_[$id];
                $company_data = $this->company_data();

                $invoice_number .= '_'.$invoice_db_data->invoice_number;
...
EOD;
            ...
            <h3 style="font-weight:normal; line-height: normal;">
                <span style="color:$text_color;">INVOICE</span> #$invoice_db_data->invoice_number
            </h3>
            </div>
EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html'=>$invoice_title, 'align'=>'R'));
...
EOD;
                ...
                $invoice_info = <<<EOD
            <div style="font-size:8; font-weight:normal; color:$text_color;">
            <strong>Invoice date:</strong> $invoice_db_data->date <br>
            <strong>Due date:</strong> $invoice_db_data->due_date <br>
            <strong>Ref No:</strong> $invoice_db_data->reference<br>
            <strong>Amount due:</strong> $invoice_db_data->currency_symbol $invoice_db_data->amount_outstanding <br>
            </div>
EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html'=>$invoice_info, 'align'=>'R', 'y'=>50));

                # contact info
                $client_org = $invoice_db_data->contact->organisation;
                $client_addr = $invoice_db_data->contact->address;
                $contact_info = <<<EOD
            <div style="font-size:8; font-weight:normal; color:$text_color;">
...
EOD;
                ...
                $invoice_items = '';
                foreach($invoice_db_data->items as $key => $item)
                {
                    $invoice_items .= '
                <tr>
                    ...
                    <td style="width:50%;">
                        <table align="right" style="width:65%; font-weight:bold; margin-left:300px; text-align:right;">
                            <tr style="font-size:10px; font-weight:normal; line-height: normal;">
                                ...
                                <td style="color:$text_color2;"><h3 style="font-weight:normal">$invoice_db_data->currency_symbol $invoice_db_data->total_amount</h3></td>
                            </tr>
                        </table>
                    </td>
                </tr>
            ...
            <table style="width:100%; border-top:1px solid #e8e8e8;">
                ...
                <tr style="font-size:6px; font-weight:normal; line-height: normal;">
                    <td style="width:50%;">$invoice_db_data->terms</td>
                    <td style="width:50%;">$invoice_db_data->closing_note</td>
                </tr>
            </table>

EOD;
                $this->CI->mytcpdf->write_htmlcell(array('html'=>$actual_invoice, 'x'=>12, 'y'=>80, 'ln'=>1));
            }
        }
        /*END OF INVOICES LOOP*/

        # set PDF font
        $this->CI->tcpdf->SetFont(PDF_FONT_NAME_MAIN, '', 10);

        //$file_path = FCPATH.'downloads/pdf/'.md5('invoice'.$invoice_number).'.pdf';
        //$file_path = FCPATH . 'assets/pdf/' . md5('invoice' . $invoice_number) . '.pdf';
        if(is_array($original_ids))
        {
            $file_path = FCPATH . 'assets/pdf/' .'bulk_invoices_' . create_slug(current_datetime()) . '.pdf';
        }
        else
        {
            $file_path = FCPATH . 'assets/pdf/' . $invoice_number . '_' . create_slug(current_datetime()) . '.pdf';
        }
        ...
        if(file_exists($file_path))
        {
            ...
            $return['message'] = 'invoice pdf(s) ready for download';
        }
        else
        {
            $return['message'] = 'could not export invoice pdf(s).';
        }
    ...
    public function invoice_data($invoice_id = null)
    {
        ...
        $result = $this->CI->template_model->read($params, $invoice_id);
