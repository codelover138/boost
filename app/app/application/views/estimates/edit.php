...
?>
 <!-- content area -->
   <form method="put" action="<?php echo base_api_url('estimates/'.$estimate_data['id']); ?>"  autocomplete="off">
   <input type="hidden" name="status" value="<?php echo ($estimate_data['status'] ? $estimate_data['status']:'draft'); ?>" />
    <div class="container-fluid bg-white doc-spaced">
      ...
      <div class="form_section">
          <div class="form-group col-xs-12 col-sm-6">
            ...
            <div class="col-sm-9 pull-right-sm no-gutter-xs">
                <?php 
					...
					$contact_data['current_id'] = @$estimate_data['contact']['id'];
          ...
          <div class="form-group col-xs-12 col-sm-6  pull-right-sm clear-right-sm">
            ...
            <div class="col-sm-9  pull-right-sm no-gutter-xs">
                <input type="text" class="form-control datepicker" name="date" id="date" value="<?php echo $estimate_data['date']; ?>">
            </div>
          </div>                    
          <?php 
                ...
                <select id="currency_id" name="currency_id" class="selectpicker full-width">
                	<?php						
						foreach($request['piggyback']['currencies'] as $currency_data){
							if($estimate_data['currency_id'] ==  $currency_data['id']){
		  ...
		  ?>
          <div class="form-group col-xs-12 col-sm-6  pull-right-sm clear-right-sm">
            <label for="due_date" class="control-label pull-left-sm">Expiration Date</label>
            <div class="col-sm-9  pull-right-sm  no-gutter-xs">
                <input type="text" class="form-control datepicker" name="due_date" id="due_date"  value="<?php echo $estimate_data['due_date']; ?>">
            </div>
          </div>  
          ...
          <div class="form-group col-xs-12 col-sm-6 pull-left-sm clear-left-sm">
            ...
            <div class="col-sm-9  pull-right-sm no-gutter-xs">
                <input value="<?php echo $estimate_data['discount_percentage']; ?>" name="discount_percentage" class="form-control" id="discount_percentage" placeholder="%">
            </div>
          </div>
          
          <div class="form-group col-xs-12 col-sm-6  pull-right-sm clear-right-sm">
            <label for="estimate_number" class="control-label pull-left-sm">Estimate No</label>
            <div class="col-sm-9 pull-right-sm no-gutter-xs">
               <?php
			   		if($estimate_data['status'] != 'draft'){
			   ?>
                		<input disabled="disabled" type="text" class="form-control" id="estimate_number" value="<?php echo $estimate_data['estimate_number']; ?>">	
               <?php
					}else{
			   ?>
               			<input type="text" class="form-control" name="estimate_number" id="estimate_number" value="<?php echo $estimate_data['estimate_number']; ?>">	
               <?php
					}
			   ?>
            </div>
          ...
          <div class="form-group col-xs-12 col-sm-6  pull-right-sm clear-right-sm">
            ...
            <div class="col-sm-9  pull-right-sm  no-gutter-xs">
                <input type="text" class="form-control" name="reference" id="reference" placeholder="Reference" value="<?php echo $estimate_data['reference']; ?>">
            </div>
          </div> 
          
           <?php 
		  # disabled until a future time
		  # this was disabled for now as I am not sure if estimates should have a reminder or not
		  if(false){ 
		  ?>
           <div class="form-group col-xs-12 col-sm-6 pull-left-sm clear-left-sm">
             <div class="col-sm-9  pull-right-sm no-gutter-xs">
             	<input type="hidden" id="reminder" name="reminder" value="<?php echo $estimate_data['reminder']; ?>" />
                <a id="reminder_cta" href="<?php echo base_url('estimates/modal/reminder/'.$estimate_data['reminder'].'/'.$estimate_data['id']); ?>" class="open-modal">
                	<?php
						if($estimate_data['reminder'] == '0'){
							...
							echo 'Reminder sent every '.$estimate_data['reminder'].' Days';
						}
                 	?>  
                </a>
             </div>
     ...
     <div class="form_section ">
         <div class="container-fluid itemList">
            ...
            <?php 
				foreach($estimate_data['items'] as $item_data['data']){
     ...
     <div class="form_section">
         <div class="form-group col-xs-12 col-sm-6   pull-left-sm clear-left-sm">
              <div class="form-group col-xs-12  no-gutter-xs">
               ...
               <textarea rows="6" class="form-control" name="terms" placeholder="Terms (or Banking Details)"><?php echo $estimate_data['terms']; ?></textarea>                        
              </div>
         </div>
         <div class="form-group col-xs-12 col-sm-6 pull-right-sm clear-right-sm">
               <div class="form-group col-xs-12 no-gutter-xs">
               <label for="closing_note" class="control-label">Closing Note:</label>                       
               <textarea rows="6" class="form-control" name="closing_note" placeholder="Add notes visible to client"><?php echo $estimate_data['closing_note']; ?></textarea>                        
              </div>
         </div>
     </div>
    ...
    <div class="form_section container-fluid" style="text-align:right;">
         <button data-redirect-url="<?php echo base_url('estimates'); ?>" data-modal-url="<?php echo base_url('modal/notice'); ?>" data-close-delay-seconds="1" data-modal-heading="Success" data-modal-body="Saved Successfully. " type="button" class="btn btn-default saveButton padded saveFormData">Save</button>
         <button data-redirect-url="<?php echo base_url('estimates'); ?>" data-modal-url="<?php echo base_url('estimates/modal/send'); ?>" type="button" class="btn btn-success saveButton saveFormData">Send</button>
    </div>
