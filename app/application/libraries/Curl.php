	...
	private function get_request_headers()
{
	...
	public function api_call($method, $resource, $data = NULL)
    {
		...
		if($response_info['http_code'] == 200){
		...
		}elseif($response_info['http_code'] == 402){
			$decoded = json_decode($output, true);
			...
			if(isset($decoded['message']) && is_array($decoded['message']) && !empty($decoded['message'][0])){
				$message = $decoded['message'][0];
			}

			if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])){
				http_response_code(402);
				if(!isset($decoded['status'])){
					$decoded['status'] = 'SUBSCRIPTION_REQUIRED';
				}
				if(!isset($decoded['message'])){
					$decoded['message'] = array($message);
				}
				echo json_encode($decoded);
				exit;
			}

			header("location:".base_url('billing').'?subscription_notice='.urlencode($message));
			exit;
		}else{		
			http_response_code($response_info['http_code']);
			print_r($output);
