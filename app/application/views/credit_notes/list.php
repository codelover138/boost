...
<div class="mainFilterContainer">
    <div class="container-fluid filters">
        ...
        <select class="selectpicker" id="bulkActions">
            ...
            <option value="<?php echo base_url('credit_notes/bulk/mark_sent'); ?>">Mark as Sent</option>
            <option value="<?php echo base_url('credit_notes/bulk/export_pdf'); ?>">Export as PDF</option>
            <?php
				#disabled untl archived section is built
				if(false){
			?>
            <option value="<?php echo base_url('credit_notes/bulk/archive'); ?>">Archive</option>
            <?php
			// end disabled
			}
			?> 
        ...
        <table class="table listInvoiceTable" style="font-weight:bold;">
          ...
          <?php 
			  ...
			  foreach($request['data'] as $item_key => $item_data){
				 // var_dump($item_data);
				  $this->load->view('credit_notes/snippets/list_row',$item_data); 
			  }
		  ?>
        </table>
   <!--  </div> -->
