...
function ajaxErrorActions(t, e, i, n) {
    ...
    else if (402 == e.status) {
        var msg = "Your subscription has been cancelled or expired. Please renew to continue using Boost Accounting.";
        try {
            var parsed402 = jQuery.parseJSON(e.responseText);
            if (parsed402 && parsed402.message && parsed402.message[0]) {
                msg = parsed402.message[0];
            }
        } catch(ex) {}
        window.location = base_url + "billing?subscription_notice=" + encodeURIComponent(msg);
...
function redirectAfterLogin(t, accountName) {
    $.ajax({
        url: api_base_url + "billing/status",
        method: "GET",
        headers: {
            Auth: getCookie("auth"),
            Session: getCookie("session_id"),
        ...
        var subscription = response && response.data && response.data.subscription ? response.data.subscription : {};
        var status = subscription.status ? String(subscription.status).toLowerCase() : "";
        var graceEndsAt = subscription.grace_period_ends_at ? Date.parse(subscription.grace_period_ends_at.replace(" ", "T")) : NaN;
        var hasGraceAccess = status === "grace_period" || (!isNaN(graceEndsAt) && graceEndsAt > Date.now());
        var mustRenew = subscription.can_pay === true && (
            status === "cancelled" ||
            status === "expired" ||
            status === "past_due"
        ) && !hasGraceAccess;
        if (mustRenew) {
            var msg = subscription.access_message || "Your subscription has been cancelled or expired. Please renew to continue using Boost Accounting.";
            window.location = base_url + "billing?subscription_notice=" + encodeURIComponent(msg);
            return;
        }
        finishLoginRedirect(t);
    }).fail(function(xhr) {
        if (xhr.status == 402) {
            var msg = "Your subscription has been cancelled or expired. Please renew to continue using Boost Accounting.";
            try {
...
function finishLoginRedirect(t) {
    if ($(t.currentTarget).hasClass("landing")) {
        window.location = "" != getUrlValue("redirAddress") ? decodeURIComponent(getUrlValue("redirAddress")) : base_url;
    } else {
        $("#main_modal").modal("hide");
    }
    if (ajaxUrl != "") {
